...
class BreakoutStrategy:
    def __init__(self, lookback: int | None = None):
        ...
        self.min_price_move_pct = cfg.get("breakout", {}).get("min_price_move_pct", 0.5)
    ...
    def check_breakout(self, symbol: str, df: pd.DataFrame) -> Signal | None:
        ...
        if last["close"] > recent_high and price_move_pct >= float(self.min_price_move_pct):
            return Signal(symbol=symbol, type="BREAKOUT", price=float(last["close"]), time=datetime.utcnow(), meta={"recent_high": float(recent_high), "price_move_pct": price_move_pct})
        return None
